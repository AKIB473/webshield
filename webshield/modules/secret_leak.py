...
SECRET_PATTERNS: List[Tuple[str, str, Severity, bool]] = [
    ...
    # AI / LLM providers (2025)
    ("OpenAI API Key",             r"sk-[A-Za-z0-9]{20}T3BlbkFJ[A-Za-z0-9]{20}",                  Severity.CRITICAL, True),
    ("OpenAI API Key (new fmt)",   r"sk-proj-[A-Za-z0-9_-]{50,}",                                  Severity.CRITICAL, True),
    ("Anthropic API Key",          r"sk-ant-api[0-9]{2}-[A-Za-z0-9_-]{93}",                        Severity.CRITICAL, True),
    ("HuggingFace Token",          r"hf_[A-Za-z0-9]{34,}",                                         Severity.CRITICAL, True),
    ("Cohere API Key",             r"(?i)co-[A-Za-z0-9]{40,}",                                     Severity.HIGH,     True),
    # Infrastructure / CDN / Hosting (2025)
    ("Cloudflare API Token",       r"[A-Za-z0-9_-]{40}cloudflare|cloudflare[A-Za-z0-9_-]{40}",     Severity.CRITICAL, True),
    ("Vercel Token",               r"(?i)vercel[_\s=:\"']+([A-Za-z0-9_-]{24,})",                   Severity.HIGH,     False),
    ("Netlify Token",              r"(?i)netlify[_\s=:\"']+([A-Za-z0-9_-]{24,})",                   Severity.HIGH,     False),
    ("DigitalOcean Token",         r"dop_v1_[a-f0-9]{64}",                                         Severity.CRITICAL, True),
    ("Supabase Key",               r"eyJ[A-Za-z0-9_-]{50,}\.[A-Za-z0-9_-]{50,}\.[A-Za-z0-9_-]{43,}", Severity.HIGH, True),
    # Communications
    ("Telegram Bot Token",         r"[0-9]{8,10}:[A-Za-z0-9_-]{35}",                               Severity.CRITICAL, True),
    ("Discord Bot Token",          r"[MN][A-Za-z0-9]{23}\.[A-Za-z0-9_-]{6}\.[A-Za-z0-9_-]{27,}",   Severity.CRITICAL, True),
]
...
def scan(url: str, timeout: float = 10.0) -> List[Finding]:
    ...
    # Pages to scan for secrets (base + common disclosure paths)
    pages_to_scan = [url]
    extra_paths = ["/about", "/config", "/debug", "/info", "/status",
                   "/api/config", "/api/v1/config", "/app", "/env"]
    ...
    with get_client(timeout=timeout) as client:
        ...
        for extra in extra_paths:
            try:
                r = client.get(base + extra)
                if r.status_code == 200 and len(r.text) > 50:
                    findings.extend(_scan_content(r.text, base + extra))
            except Exception:
                continue
        ...
        for match in _re.finditer(
            ...
            if js_path.startswith("http"):
                ...
                js_urls.append(f"{base}{js_path}")

        for js_url in js_urls[:8]:  # check first 8 JS files
            try:
