...
def _sanitize(text: str) -> str:
    """Remove surrogate characters and HTML-unsafe bytes that break UTF-8 writing."""
    if not text:
        return text
    # Replace surrogate pairs / lone surrogates
...
def _finding_html(f, idx: int) -> str:
    ...
    code_section = ""
    if f.code_fix:
        escaped = _sanitize(f.code_fix).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        code_section = f"""
    ...
    desc_s = _sanitize(f.description)
    evidence_s = _sanitize(f.evidence[:300]) if f.evidence else ""
    remediation_s = _sanitize(f.remediation) if f.remediation else ""
    ...
    <div class="finding" style="border-left:4px solid {sc}; background:{sb};">
      <div class="finding-header">
        ...
        {cvss_section}
        <span class="finding-title">{title_s}</span>
      </div>
      <p class="finding-desc">{desc_s}</p>
      {"<div class='evidence'><strong>Evidence:</strong> " + evidence_s + "</div>" if evidence_s else ""}
      {"<div class='remediation'><strong>🔧 How to Fix:</strong> " + remediation_s + "</div>" if remediation_s else ""}
      {code_section}
      {ref_section}
    </div>"""
...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    [data-theme="dark"] .score-section {{ background: #1e293b; }}
    [data-theme="dark"] .summary-card {{ background: #1e293b; }}
    [data-theme="dark"] .finding {{ filter: brightness(0.92); }}
    [data-theme="dark"] .evidence {{ background: rgba(255,255,255,0.07); color: #94a3b8; }}
    [data-theme="dark"] .remediation {{ background: rgba(22,163,74,0.15); color: #86efac; }}
    [data-theme="dark"] .code-block pre {{ background: #020617; }}
...
<body>
  <button class="theme-btn" onclick="toggleTheme()" title="Toggle dark/light mode">🌙 Dark</button>
  <script>
  ...
  <div class="footer">
    WebShield v1.5.0 &bull; by AKIBUZZAMAN AKIB &bull;
    ...
    # errors='replace' as final safety net against any remaining surrogates
    path.write_text(
        html.encode("utf-8", errors="replace").decode("utf-8"),
        encoding="utf-8"
    )
