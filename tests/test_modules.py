...
def test_cookies_all_flags_present():
    ...
    with patch("webshield.modules.cookies.get_client") as mock_client:
        mock_client.return_value.__enter__.return_value.get.return_value = mock_resp
        findings = cookies.scan("https://example.com")
    # Only flag HIGH/MEDIUM issues — LOW prefix recommendations are informational
    flag_issues = [f for f in findings if "Missing" in f.title and f.severity.value in ("HIGH", "MEDIUM", "CRITICAL")]
